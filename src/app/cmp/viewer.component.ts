...
import { Prefereneces, PlayListItem, DirectoryEntry, DirectoryGroup } from '../cls/index';
...
export class ViewerComponent implements OnInit, OnDestroy {
    ...
    public imageCheck(): boolean {
        let imgCheck: boolean = false;
        if ((this.preferences.groups != null) && (this.preferences.groups.length > 0)) {
            for (let j = 0; j < this.preferences.groups.length; j++) {
                if (this.preferences.groups[j].name == this.preferences.currentGroup) {
                    if ((this.preferences.groups[j].directories != null) && (this.preferences.groups[j].directories.length > 0)) {
                        imgCheck = true;
                    }
                    else {
                        imgCheck = false;
    ...
    public createPlaylist(grpChange: boolean): void {
        if (this.imageCheck()) {
            this.pause();
            ...
            for (let j = 0; j < this.preferences.groups.length; j++) {
                if (this.preferences.groups[j].name == this.preferences.currentGroup) {
                    for (let i = 0; i < this.preferences.groups[j].directories.length; i++) {
                        if ((this.preferences.groups[j].directories[i].path != null) && (this.preferences.groups[j].directories[i].path != "")) {
                            this.addDirectoryFiles(this.preferences.groups[j].directories[i].path, this.preferences.groups[j].directories[i].include);
                        }
                    }
                }
            }
